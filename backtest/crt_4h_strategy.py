...
CRT-4h Strategy
===============
CRT backtest on the 4H timeframe -- the engine ONLY, with NO v2/v3 filter stage.

Detects CRT setups on 4H candles, uses 1H candles for entry fill and trade
...
Data requirements:
    {SYMBOL}_4h_all.csv   -- 4H OHLC data (setup detection + PDH/PDL)
    {SYMBOL}_1h_all.csv   -- 1H OHLC data (entry fill + trade resolution)

Outputs:
    crt_4h_trades_final.csv          <- trade-level rows
    crt_4h_without_filter_summary.csv <- per-year summary table + TOTAL row

Run:
    python crt_4h_strategy.py
"""
...
TRADE_START_H = 7
TRADE_END_H = 18
# Allow up to 8 hours (2 x 4H candles) between setup close and entry fill
MAX_SETUP_TO_ENTRY_H = 8
...
OUT_CSV = os.path.join(HERE, "crt_4h_trades_final.csv")
...
SUMMARY_CSV   = os.path.join(HERE, "crt_4h_without_filter_summary.csv")
...
def run_engine():
    """Detect + gate + resolve every 4H CRT setup.

    4H candles are used for setup detection and PDH/PDL. 1H candles are used
    for fine-grained entry fill and trade resolution within each day.
    """
    # 4H candles: setup detection + daily OHLC aggregation
    h4 = load(_data(f"{SYMBOL}_4h_all.csv"))
    by_day, daily = {}, {}
    for c in h4:
        b = day_bucket(c["t"])
    ...
    # 1H candles: entry fill and trade resolution (finer resolution than 4H)
    h1_series = Series(load(_data(f"{SYMBOL}_1h_all.csv")))
    bias_map = fetch_bias()
    ...
    while d <= end:
        ...
        if prev and len(cands) >= 2:
            ...
            for kind, c1, c2 in detect(cands, prev["h"], prev["l"]):
                ...
                # setup available after C2 closes (4H after C2 open)
                setup_found = c2["t"] + FOUR_HOUR_MS
                fill = find_fill(h1_series, direction, entry, setup_found, day_end)
                ...
                row["trade taken"] = "Yes"
                st["taken"] += 1
                outcome, close_ms = resolve(h1_series, direction, sl, tp, fill, day_end)
                if outcome == "TP":
    ...
    candles_by_ms = {c["t"]: c for c in h4}
    daily_by_date = {ist(b).strftime("%Y-%m-%d"): {"h": v["h"], "l": v["l"], "c": v["c"]}
                     for b, v in daily.items()}
    return rows, daily_by_date, h1_series, candles_by_ms
...
# ============================================================================
def main():
    print("=== CRT-4H: STAGE 1 ENGINE (no v2/v3 filters) ===")
    engine_rows, daily, h1_series, candles = run_engine()

    print("=== CRT-4H: SUMMARY (per year) ===")
    summary_rows = []
