"""
strategies/crt_4h.py — live CRT strategy on 4H timeframe (fixed-stop mode).

Detection: 2-candle CRT pattern on 4H candles.
...
def detect(cands):
    """2-candle CRT pattern on 4H candles. Returns list of ('Bullish'|'Bearish', c1, c2)."""
    out = []
...
async def fetch_node_bias(symbol):
    ...
    print(f"[crt_4h] node bias fetch failed for {symbol} after "
