"""
strategies/sweep_1h_enhanced.py — live port of the ENHANCED 1H Sweep Swing
strategy (backtest: enhanced_1hr.py).

Same enhanced sweep rules as sweep_4h_enhanced, run on 1H candles.
Detection (per consecutive 1H candle pair C1, C2), entry always at C2 open.
C1 may be green OR red — each direction has a branch per C1 colour (a doji
C1, open == close, never qualifies):
...
from .sweep_4h_enhanced import detect_enhanced
...
class Sweep1HEnhancedStrategy(Strategy):
    name = "sweep_1h_enhanced"
    label = "1H Sweep Enhanced"
